...
(function() {
    ...
    const isVimModeEnabled = () => {
        return window.app?.vault?.config?.vimMode === true;
    };
    
    const addVimCursorSupport = () => {
        if (!isVimModeEnabled()) return;
        
        const editors = document.querySelectorAll('.cm-editor');
        editors.forEach(editor => {
            const vimCursorLayer = editor.querySelector('.cm-vimCursorLayer');
            if (vimCursorLayer && !vimCursorLayer.hasAttribute('data-vim-animated')) {
                vimCursorLayer.setAttribute('data-vim-animated', 'true');
                
                const observer = new MutationObserver(() => {
                    if (editor.classList.contains('cm-focused') && !vimCursorLayer.classList.contains('cm-blinkLayer')) {
                        setTimeout(() => {
                            vimCursorLayer.classList.add('cm-blinkLayer');
                        }, 350);
                    }
                });
                ...
                vimCursorLayer.addEventListener('transitionstart', () => {
                    vimCursorLayer.classList.remove('cm-blinkLayer');
                });
                
                vimCursorLayer.addEventListener('transitionend', () => {
                    setTimeout(() => {
    ...
    const initVimSupport = async () => {
        await waitForApp();
        if (isVimModeEnabled()) {
            console.log('Initializing vim cursor animation support');
            ...
            addVimCursorSupport();
            setInterval(addVimCursorSupport, 2000);
        }
    };
    
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initVimSupport);
    } else {
        initVimSupport();
    }
})();